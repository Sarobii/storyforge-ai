...
export const useMathRPGSave = (userId?: string) => {
  ...
  const getSaveKey = useCallback(
    () => (userId ? `mathRPGSave_${userId}` : "mathRPGSave_guest"),
    [userId]
  );
  ...
  const loadFromSupabase =
    useCallback(async (): Promise<MathRPGSaveData | null> => {
  ...
  const saveToSupabase = useCallback(
    async (data: MathRPGSaveData): Promise<boolean> => {
  ...
  const save = useCallback(
    async (newData?: Partial<MathRPGSaveData>): Promise<boolean> => {
      try {
  ...
  const autoSave = useCallback(
    async (updates?: Partial<MathRPGSaveData>) => {
      if (updates) {
        updateSave(updates);
  ...
  const mathAccuracy =
    saveData.totalMathProblemsAttempted > 0
      ? (saveData.totalMathProblemsCorrect /
          saveData.totalMathProblemsAttempted) *
        100
      : 0;
  ...
  return {
    ...
    isNewGame: () =>
      saveData.currentBattle === 1 && saveData.totalEnemiesDefeated === 0,
  };
};
